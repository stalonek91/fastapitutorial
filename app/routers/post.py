...
def read_sql_posts(db: Session = Depends(get_sql_db), current_user: int = Depends(oauth2.get_current_user)):
    posts = db.query(models.Post).all()
    return posts


@router.get("/{post_id}", response_model=schemas.PostResponse)
def get_post_id(post_id: int, db: Session = Depends(get_sql_db), current_user: int = Depends(oauth2.get_current_user)):
...
def add_post_pydantic(post_data: schemas.PostBase, db: Session = Depends(get_sql_db), current_user: int = Depends(oauth2.get_current_user)):
...
def delete_post(post_id: int, db: Session = Depends(get_sql_db), current_user: int = Depends(oauth2.get_current_user)):
